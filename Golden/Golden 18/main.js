...
function isWindowMaximized() {
    ...
    function buildDbFilterPanel(topics, types, skipRestore = false) {
      ...
      const allTypes = document.createElement("label");
      allTypes.innerHTML = `<input type=\"checkbox\" value=\"ALL\" checked> ALL <span style="color: #666; font-weight: normal;">(${totalFilteredQuestions} question${totalFilteredQuestions === 1 ? '' : 's'})</span>`;
      typeDiv.appendChild(allTypes);
          ...
          if (availableQuestionCount > 0) {
            ...
            if (selectAllTopics && selectAllTopics.checked) {
              ...
              selectedTypes.forEach(qType => {
                const typeCountSql = `SELECT COUNT(*) FROM questions WHERE question_type = '${qType}'`;
              ...
              for (const [topic, subtopics] of selectedTopicMap) {
                ...
                const typeBreakdown = [];
                selectedTypes.forEach(qType => {
                  const subtopicConditions = subtopics.map(st => `(topic = '${topic}' AND subtopic = '${st}')`).join(' OR ');
                  const typeCountSql = `SELECT COUNT(*) FROM questions WHERE (${subtopicConditions}) AND question_type = '${qType}'`;
                  const typeCountRes = AppState.database.exec(typeCountSql);
                  const typeCount = typeCountRes[0]?.values[0][0] || 0;
                  if (typeCount > 0) {
      ...
      startBtn.addEventListener("click", () => {
        ...
        allQuestions.forEach((q, index) => {
          ...
          if (q.question_type === 'MCQ' || q.question_type === 'MCQ-Scenario' || q.question_type === 'Cohort-05-MCQ') {
            ...
            if (!valid) {
              invalidReason = `No options found (options.length = ${q.options.length})`;
              console.log(`❌ Question ${q.id} (${q.question_type}): INVALID - ${invalidReason}`);
              console.log(`   Options query result:`, optRes[0]?.values || "No results");
            }
          ...
          if (q.question_type === 'Match') {
            ...
            if (matchRes[0] && matchRes[0].values && matchRes[0].values.length > 0) {
              ...
              valid = false;
              invalidReason = `No match pairs found`;
              console.log(`❌ Question ${q.id} (${q.question_type}): INVALID - ${invalidReason}`);
            }
            q.type = 'match';
          }
          ...
          if (q.question_type === 'AssertionReason') {
            ...
            if (optRes[0]?.values.length) {
              ...
              valid = false;
              invalidReason = `No assertion-reason options found in options table`;
              console.log(`❌ Question ${q.id} (${q.question_type}): INVALID - ${invalidReason}`);
            }
            q.type = 'assertion';
          }
          ...
          if (valid) {
            ...
            invalidCount++;
            if (!invalidReason) {
              invalidReason = `Unknown validation failure for question type: ${q.question_type}`;
              console.log(`❌ Question ${q.id} (${q.question_type}): INVALID - ${invalidReason}`);
            }
          }
        });
      ...
      function updateMaxQuestions() {
        ...
        if (selectAllTopics && selectAllTopics.checked) {
          // All topics and subtopics selected
          sql = `SELECT * FROM questions WHERE question_type IN (${selectedTypes.map(t => `'${t}'`).join(',')})`;
        } else {
          // Build query based on individual subtopic selections
          const selectedSubtopics = topicDiv.querySelectorAll(".subtopic-checkbox:checked");
          ...
          sql = `SELECT * FROM questions WHERE (${conditions.join(' OR ')}) AND question_type IN (${selectedTypes.map(t => `'${t}'`).join(',')})`;
        }
        ...
        allQuestions.forEach((q, index) => {
          ...
          // MCQ and MCQ-Scenario and Cohort-05-MCQ: check if options exist
          if (q.question_type === 'MCQ' || q.question_type === 'MCQ-Scenario' || q.question_type === 'Cohort-05-MCQ') {
            const optRes = AppState.database.exec(`SELECT option_text, is_correct FROM options WHERE question_id = ${q.id}`);
            const hasOptions = optRes[0]?.values?.length > 0;
            valid = hasOptions;
            if (!valid) reason = "No options found";
          }
          // TrueFalse: check if options exist
          else if (q.question_type === 'TrueFalse') {
            const optRes = AppState.database.exec(`SELECT option_text, is_correct FROM options WHERE question_id = ${q.id}`);
            valid = optRes[0]?.values?.length > 0;
            if (!valid) reason = "No True/False options found";
          }
          // Match: check if match pairs exist
          else if (q.question_type === 'Match') {
            const matchRes = AppState.database.exec(`SELECT left_text, right_text FROM match_pairs WHERE question_id = ${q.id}`);
            valid = matchRes[0] && matchRes[0].values && matchRes[0].values.length > 0;
            if (!valid) reason = "No match pairs found";
          }
          // AssertionReason: check if options exist (using options table like other question types)
          else if (q.question_type === 'AssertionReason') {
            const optRes = AppState.database.exec(`SELECT option_text, is_correct FROM options WHERE question_id = ${q.id}`);
            valid = optRes[0]?.values?.length > 0;
            if (!valid) reason = "No assertion-reason options found in options table";
          }
          // Unknown question types - mark as invalid for now
          else {
            valid = false;
    ...
    function renderTest(questions) {
      try {
        ...
        questions.forEach((q, qIndex) => {
          ...
          // Format question text to handle Roman numerals (I., II., III., IV., etc.)
          function formatQuestionWithRomanNumerals(questionText) {
            // Check if the question contains Roman numerals pattern
            const romanNumeralPattern = /\b(I{1,3}V?|IV|V|VI{0,3}|IX|X{1,3})\.\s/g;
            
            if (romanNumeralPattern.test(questionText)) {
              // Find the position of the first Roman numeral
          ...
          const formattedQuestion = formatQuestionWithRomanNumerals(q.question);
  ...
  if (prevTryContainer) prevTryContainer.remove();
    ...
    function showInvalidQuestionsPopup(invalidQuestions, totalQuestions) {
      ...
      modal.style.cssText = `
        background: white; border-radius: 8px; max-width: 95vw; max-height: 80vh; 
      ...
      title.textContent = `Invalid Questions Report`;
      ...
      summary.style.cssText = "padding: 15px; background: #fff3e0; border-bottom: 1px solid #ddd;";
      const validCount = totalQuestions - invalidQuestions.length;
      summary.innerHTML = `
        <strong>Summary:</strong> Out of ${totalQuestions} questions matching your filters:<br>
        ✅ <span style="color: green">${validCount} valid questions</span> (will be included in test)<br>
        ❌ <span style="color: red">${invalidQuestions.length} invalid questions</span> (excluded due to missing data)
      `;
      ...
      if (invalidQuestions.length === 0) {
        ...
        thead.innerHTML = `
          <tr style="background: #f5f5f5;">
            <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 80px;">Question ID</th>
            <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 120px;">Question Type</th>
            <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 150px;">Topic</th>
            <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 150px;">Subtopic</th>
            <th style="border: 1px solid #ddd; padding: 8px; text-align: left; min-width: 300px;">Question Text</th>
            <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 250px;">Reason</th>
          </tr>
        `;
        ...
        invalidQuestions.forEach((q, index) => {
          ...
          row.innerHTML = `
            <td style="border: 1px solid #ddd; padding: 8px; font-weight: bold; color: #0078d7;">${q.id}</td>
            <td style="border: 1px solid #ddd; padding: 8px;">${q.question_type}</td>
            <td style="border: 1px solid #ddd; padding: 8px;">${q.topic}</td>
            <td style="border: 1px solid #ddd; padding: 8px;">${q.subtopic}</td>
            <td style="border: 1px solid #ddd; padding: 8px; word-wrap: break-word; white-space: pre-wrap; max-width: 400px;">${q.question_text}</td>
            <td style="border: 1px solid #ddd; padding: 8px; color: #d32f2f; word-wrap: break-word;">${q.reason}</td>
          `;
