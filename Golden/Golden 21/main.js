...
// SQL-safe string escaping utility
function escapeSQL(str) {
  return str.replace(/'/g, "''");
}

// Function to dynamically load SQL.js library
function loadSQLJS() {
...
// Load SQL.js library immediately
...
function isWindowMaximized() {
    ...
    const AppState = {
      ...
      // Store invalid questions for refresh functionality
    ...
    function buildDbFilterPanel(topics, types, skipRestore = false) {
      ...
      function updateMaxQuestions() {
        ...
        // 2.2 Broken multi-answer MCQs (some but not all correct)
        try {
          const brokenMultiAnswerSQL = `
            SELECT q.id, q.question_text, q.question_type, q.topic, q.subtopic, 'Broken multi-answer MCQ' as reason
            FROM questions q
            JOIN options o ON q.id = o.question_id
            WHERE q.question_type = 'MCQ'
            GROUP BY q.id
            HAVING SUM(CASE WHEN o.is_correct = 1 THEN 1 ELSE 0 END) > 1
               AND SUM(CASE WHEN o.is_correct = 1 THEN 1 ELSE 0 END) < COUNT(*)
          `;
          const brokenMultiResult = AppState.database.exec(brokenMultiAnswerSQL);
          if (brokenMultiResult[0]?.values) {
            brokenMultiResult[0].values.forEach(row => {
        ...
        } catch (e) { console.log("Error checking broken multi-answer MCQs:", e); }
  ...
  if (prevTryContainer) prevTryContainer.remove();
    ...
    function segregateValidationIssues(invalidQuestions) {
      ...
      invalidQuestions.forEach(q => {
        ...
        if (reason.includes("Duplicate options") || 
    ...
    function createIssueTable(issues, type) {
      ...
      tableContainer.style.cssText = "background: white; padding: 15px;";
      ...
      thead.innerHTML = `
        <tr style="background: #f5f5f5;">
          <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 80px;">ID</th>
          <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 100px;">Type</th>
          <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 120px;">Topic</th>
          <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 120px;">Subtopic</th>
          <th style="border: 1px solid #ddd; padding: 8px; text-align: left; min-width: 250px;">Question Text</th>
          <th style="border: 1px solid #ddd; padding: 8px; text-align: left; width: 200px;">${issueColumnHeader}</th>
        </tr>
      `;
      ...
      issues.forEach((issue, index) => {
        ...
        row.innerHTML = `
          ...
          <td style="border: 1px solid #ddd; padding: 8px; color: ${reasonColor}; word-wrap: break-word; font-size: 0.85em;">${issue.reason}</td>
        `;
...
${content}`], { type: 'text/plain' });
    ...
    function clearDatabaseDuplicates(overlayToClose) {
      ...
      createConfirmationModal(
        ...
        <div style="background: #f8f4e6; padding: 10px; border-radius: 4px; border-left: 3px solid #d69e2e; margin-bottom: 15px;">
          <strong>⚠️ Important:</strong> This action cannot be undone. Click "Yes, Proceed" only if you have a backup.
        </div>
        <div style="background: #e3f2fd; padding: 10px; border-radius: 4px;">
          <strong>This will remove:</strong><br>
          • Duplicate entries from options table<br>
          • Duplicate entries from match_pairs table
        </div>`,
        () => {
          // Execute cleanup immediately after single confirmation
          executeCleanupAndRefresh(overlayToClose);
        }
      );
    }

    // Function to create HTML confirmation modals
    function createConfirmationModal(title, content, onConfirm) {
      const overlay = document.createElement("div");
      overlay.style.cssText = `
        position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
        background: rgba(0,0,0,0.6); z-index: 15000; display: flex; 
        justify-content: center; align-items: center; padding: 20px; box-sizing: border-box;
      `;
      ...
      const header = document.createElement("div");
      header.style.cssText = `
        padding: 20px; background: #ff9800; color: white; text-align: center;
      `;
      header.innerHTML = `<h3 style="margin: 0; font-size: 1.2em;">${title}</h3>`;
      ...
      footer.style.cssText = `
        padding: 20px; background: #f5f5f5; display: flex; 
    ...
    // Function to execute the actual cleanup and refresh the popup
    function executeCleanupAndRefresh(overlayToClose) {
      try {
        console.log("🧹 Starting duplicate cleanup...");
        
        let deletedOptionsCount = 0;
        let deletedMatchPairsCount = 0;
        
        // Step 1: Clean duplicate options - using subqueries that work with SQL.js
        console.log("Cleaning duplicate options...");
        try {
          // Count before cleanup
          const beforeOptionsResult = AppState.database.exec("SELECT COUNT(*) as count FROM options");
          const beforeOptionsCount = beforeOptionsResult[0]?.values[0][0] || 0;
          console.log(`Before options cleanup: ${beforeOptionsCount} records`);
          
          // Count actual duplicates before deletion
          const duplicateOptionsSQL = `
            SELECT COUNT(*) as dupe_count FROM options 
            WHERE rowid NOT IN (
              SELECT MIN(rowid) 
              FROM options 
          ...
          `;
          const dupeOptionsResult = AppState.database.exec(duplicateOptionsSQL);
          const actualOptionDupes = dupeOptionsResult[0]?.values[0][0] || 0;
          console.log(`Found ${actualOptionDupes} duplicate option records to delete`);
          
          // Delete duplicates using rowid and subquery (SQL.js compatible)
          ...
          `;
          console.log("Executing options delete SQL:", deleteOptionsSQL);
          AppState.database.exec(deleteOptionsSQL);
          
          // Count after cleanup
          const afterOptionsResult = AppState.database.exec("SELECT COUNT(*) as count FROM options");
          const afterOptionsCount = afterOptionsResult[0]?.values[0][0] || 0;
          deletedOptionsCount = beforeOptionsCount - afterOptionsCount;
          
          console.log(`Options cleanup: deleted ${deletedOptionsCount} duplicates (expected: ${actualOptionDupes})`);
        } catch (optError) {
          console.error("Error cleaning options:", optError);
        }
        
        // Step 2: Clean duplicate match_pairs
        console.log("Cleaning duplicate match_pairs...");
        try {
          // Count before cleanup
          const beforeMatchResult = AppState.database.exec("SELECT COUNT(*) as count FROM match_pairs");
          const beforeMatchCount = beforeMatchResult[0]?.values[0][0] || 0;
          console.log(`Before match pairs cleanup: ${beforeMatchCount} records`);
          
          // Count actual duplicates before deletion
          const duplicateMatchSQL = `
            SELECT COUNT(*) as dupe_count FROM match_pairs 
            WHERE rowid NOT IN (
              SELECT MIN(rowid) 
              FROM match_pairs 
          ...
          `;
          const dupeMatchResult = AppState.database.exec(duplicateMatchSQL);
          const actualMatchDupes = dupeMatchResult[0]?.values[0][0] || 0;
          console.log(`Found ${actualMatchDupes} duplicate match pair records to delete`);
          
          // Delete duplicates using rowid and subquery (SQL.js compatible)
          const deleteMatchPairsSQL = `
          ...
          `;
          console.log("Executing match pairs delete SQL:", deleteMatchPairsSQL);
          AppState.database.exec(deleteMatchPairsSQL);
          
          // Count after cleanup
          const afterMatchResult = AppState.database.exec("SELECT COUNT(*) as count FROM match_pairs");
          const afterMatchCount = afterMatchResult[0]?.values[0][0] || 0;
          deletedMatchPairsCount = beforeMatchCount - afterMatchCount;
          
          console.log(`Match pairs cleanup: deleted ${deletedMatchPairsCount} duplicates (expected: ${actualMatchDupes})`);
        } catch (matchError) {
          console.error("Error cleaning match_pairs:", matchError);
        }
        
        console.log("✅ Duplicate cleanup completed successfully!");
        console.log(`Total deleted: ${deletedOptionsCount} option duplicates, ${deletedMatchPairsCount} match pair duplicates`);
        
        // Close the original invalid questions popup
        if (overlayToClose && overlayToClose.parentNode) {
          document.body.removeChild(overlayToClose);
        }
        
        // Show success modal and automatically refresh
        createSuccessModal(deletedOptionsCount, deletedMatchPairsCount);
    ...
    // Function to show success and automatically refresh the invalid questions popup
    function createSuccessModal(deletedOptionsCount, deletedMatchPairsCount) {
      const totalDeleted = deletedOptionsCount + deletedMatchPairsCount;
      
      if (totalDeleted > 0) {
        // Show success modal with auto-refresh
        createConfirmationModal(
          "✅ Cleanup Successful",
          ...
          <div style="margin-bottom: 15px;">
            ...
            • Deleted ${deletedOptionsCount} duplicate option entries<br>
            • Deleted ${deletedMatchPairsCount} duplicate match pair entries
          </div>
          <div style="background: #e8f5e8; padding: 10px; border-radius: 4px; border-left: 3px solid #28a745;">
            The invalid questions report will now be refreshed automatically.
          </div>`,
          () => {
            // Auto-refresh the invalid questions popup
            refreshInvalidQuestionsPopup();
          }
        );
      } else {
        // No duplicates found - still refresh but with different message
        createConfirmationModal(
          "ℹ️ No Duplicates Found",
          `<div style="color: #17a2b8; font-weight: bold; margin-bottom: 15px;">
            No duplicate entries were found to remove.
          </div>
          <div style="margin-bottom: 15px;">
            Your database appears to be clean of duplicates already.
          </div>
          <div style="background: #e3f2fd; padding: 10px; border-radius: 4px; border-left: 3px solid #17a2b8;">
            The invalid questions report will be refreshed to confirm.
          </div>`,
          () => {
            // Auto-refresh the invalid questions popup
            refreshInvalidQuestionsPopup();
    ...
    function createErrorModal(errorMessage) {
      ...
      modal.innerHTML = `
        <div style="padding: 20px; background: #dc3545; color: white; text-align: center;">
          <h3 style="margin: 0;">❌ Cleanup Failed</h3>
        </div>
        <div style="padding: 20px;">
          <div style="color: #d32f2f; margin-bottom: 15px;">
            <strong>Error during duplicate cleanup:</strong>
          </div>
          <div style="background: #f8f4e6; padding: 10px; border-radius: 4px; margin-bottom: 15px; font-family: monospace; font-size: 0.9em;">
            ${errorMessage}
          </div>
          <div>Your database was not modified. Please check the console for more details.</div>
        </div>
        <div style="padding: 20px; background: #f5f5f5; text-align: right;">
    ...
    // Function to refresh the invalid questions popup with updated data
    function refreshInvalidQuestionsPopup() {
      // Re-run the validation to get updated invalid questions list
      if (AppState.database && AppState.lastQueryAllQuestions) {
        console.log("Refreshing invalid questions report...");
        
        // Get all questions again and validate
        let invalidQuestions = [];
        AppState.lastQueryAllQuestions.forEach((q, index) => {
          let valid = false;
          let reason = "";
          
          // MCQ and MCQ-Scenario and Cohort-05-MCQ: check if options exist and for duplicates
          if (q.question_type === 'MCQ' || q.question_type === 'MCQ-Scenario' || q.question_type === 'Cohort-05-MCQ') {
            const optRes = AppState.database.exec(`SELECT option_text, is_correct FROM options WHERE question_id = ${q.id}`);
            const hasOptions = optRes[0]?.values?.length > 0;
            if (!hasOptions) {
              valid = false;
              reason = "No options found";
            } else {
              // Check for duplicate options
              const optionTexts = optRes[0].values.map(v => v[0]);
              const uniqueOptions = [...new Set(optionTexts)];
              if (optionTexts.length !== uniqueOptions.length) {
                valid = false;
                const duplicates = optionTexts.filter((item, index) => optionTexts.indexOf(item) !== index);
                reason = `Duplicate options found: ${[...new Set(duplicates)].join(', ')}`;
              } else {
                valid = true;
              }
            }
          ...
          // Similar validation for other question types...
            ...
            if (!hasOptions) {
              valid = false;
              reason = "No True/False options found";
            } else {
              const dbOptions = optRes[0].values.map(v => v[0]);
              const uniqueDbOptions = [...new Set(dbOptions)];
              if (dbOptions.length !== uniqueDbOptions.length) {
                valid = false;
                const duplicates = dbOptions.filter((item, index) => dbOptions.indexOf(item) !== index);
                reason = `Duplicate TrueFalse options in database: ${[...new Set(duplicates)].join(', ')}`;
              } else {
                valid = true;
              }
            }
            ...
            const matchRes = AppState.database.exec(`SELECT left_text, right_text FROM match_pairs WHERE question_id = ${q.id}`);
            const hasPairs = matchRes[0] && matchRes[0].values && matchRes[0].values.length > 0;
            if (!hasPairs) {
              valid = false;
              reason = "No match pairs found";
            } else {
              const leftTexts = matchRes[0].values.map(v => v[0]);
              const rightTexts = matchRes[0].values.map(v => v[1]);
              const uniqueLeftTexts = [...new Set(leftTexts)];
              ...
              if (leftTexts.length !== uniqueLeftTexts.length) {
                valid = false;
                const duplicateLefts = leftTexts.filter((item, index) => leftTexts.indexOf(item) !== index);
                reason = `Duplicate left side items: ${[...new Set(duplicateLefts)].join(', ')}`;
              } else if (rightTexts.length !== uniqueRightTexts.length) {
                valid = false;
                const duplicateRights = rightTexts.filter((item, index) => rightTexts.indexOf(item) !== index);
                reason = `Duplicate right side items: ${[...new Set(duplicateRights)].join(', ')}`;
              } else {
                valid = true;
              }
            }
            ...
            if (!hasOptions) {
              valid = false;
              reason = "No assertion-reason options found in options table";
            } else {
              const allOptions = optRes[0].values.map(row => row[0]);
              const uniqueOptions = [...new Set(allOptions)];
              if (allOptions.length !== uniqueOptions.length) {
                valid = false;
                const duplicates = allOptions.filter((item, index) => allOptions.indexOf(item) !== index);
                reason = `Duplicate AssertionReason options: ${[...new Set(duplicates)].join(', ')}`;
              } else {
                valid = true;
              }
            }
            ...
            valid = false;
            reason = `Unknown question type: ${q.question_type}`;
          }
        ...
        // Store updated invalid questions and show refreshed popup
        AppState.storedInvalidQuestions = invalidQuestions;
        AppState.currentInvalidQuestions = invalidQuestions;
        const totalQuestions = AppState.lastQueryAllQuestions.length;
        
        console.log(`Validation results: ${invalidQuestions.length} invalid questions out of ${totalQuestions} total`);
        
        // Show the updated popup (this will replace any existing one)
        showInvalidQuestionsPopup(invalidQuestions, totalQuestions);
      } else {
        console.error("Cannot refresh: Missing database or query data");
      }
    }